...
static char *cond2jmp_insn(const JmpCond jmp_cond) {
    switch (jmp_cond) {
    case JmpCond_LT: return "jl";
    case JmpCond_GT: return "jg";
    case JmpCond_EQ: return "je";
    case JmpCond_GE: return "jge";
    case JmpCond_LE: return "jle";
    case JmpCond_NE: return "jne";
    }
    unreachable;
}

static void
gen_asm_instr(const Instr instr, const Func func, StrPool strs, FILE *file) {
    switch (instr.op) {
    case Op_LBL: // .lbl[a]:
        fprintf(file, ".L%u:\n", instr.a);
        break;
